...
def Draw(pred, features, poi, mark_poi=True, name="fig2.png", f1_name="feature 1", f2_name="feature 2"):
    """ some plotting code designed to help you visualize your clusters """

    # plot each cluster with a different color--add more colors for
    # drawing more than five clusters
    colors = ["b", "c", "k", "m", "g"]
    for ii in enumerate(pred):
...
# max and min values taken by the "exercised_stock_options" and "salary" features
eso = []
salary = []
...
for users in data_dict:
    ...
    if val == 'NaN':
        continue
    eso.append(float(val))
print "Max exercised_stock_options:", max(eso)
print "Min exercised_stock_options:\n", min(eso)
print "Max salary:", max(salary)
print "Min salary:\n", min(salary)

eso = [min(eso), 1000000.0, max(eso)]
eso = numpy.array([[e] for e in eso])
eso_scaler = MinMaxScaler()
rescaled_eso = eso_scaler.fit_transform(eso)
print "Rescaled eso:\n", rescaled_eso
...
try:
    Draw(pred, finance_features, poi, mark_poi=True, name="fig2.pdf", f1_name=feature_1, f2_name=feature_2)
except NameError:
